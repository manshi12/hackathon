# api_server.py
# FastAPI REST API for Email Agent Dashboard
...
# Import your existing agent (make sure it's in the same directory)
import mvp_agent
...
class EmailSummary(BaseModel):
    ...
    importance: str
    category: str
    status: str  # processed, pending, sent, error
...
async def lifespan(app: FastAPI):
    ...
    agent_running = False
    print("Email Agent API shutting down...")

app = FastAPI(
    title="Email Agent Dashboard API",
    description="REST API for Email Agent Management",
    version="1.0.0",
    lifespan=lifespan
)
...
# Enhanced agent runner with stats tracking
def run_agent_with_stats():
    ...
    while agent_running:
        try:
            ...
            original_process_one = mvp_agent.process_one_message
            original_smtp_send = mvp_agent.smtp_send
            
            def tracked_process_one(msg):
                result = original_process_one(msg)
                # Update stats
            ...
            def tracked_smtp_send(*args, **kwargs):
                result = original_smtp_send(*args, **kwargs)
                email_stats["sent_today"] += 1
                return result
            
            # Monkey patch with tracking
...
async def start_agent(background_tasks: BackgroundTasks):
    ...
    agent_running = True
    agent_thread = threading.Thread(target=run_agent_with_stats, daemon=True)
    agent_thread.start()
...
async def handle_pending_email(email_id: str, action: EmailAction):
    """Handle action on pending email (approve, reject, modify)"""
    try:
        ...
        if action.action == "approve":
            ...
            if not mvp_agent.DRY_RUN:
                # Extract sender from the email item
                sender = email_item["from"]
                to_addr = sender.split('<')[-1].strip('>')  if '<' in sender else sender
                mvp_agent.smtp_send(to_addr, reply_subject, reply_body)
            
            # Remove from pending
            mvp_agent.PENDING_REVIEW.pop(email_index)
            mvp_agent.save_state()
            
            return {"message": f"Email {'approved and sent' if not mvp_agent.DRY_RUN else 'approved (DRY RUN)'}"}
            ...
            if not mvp_agent.DRY_RUN:
                sender = email_item["from"]
                to_addr = sender.split('<')[-1].strip('>')  if '<' in sender else sender
                mvp_agent.smtp_send(to_addr, action.reply_subject, action.reply_body)
            
            # Remove from pending
            mvp_agent.PENDING_REVIEW.pop(email_index)
            mvp_agent.save_state()
            
            return {"message": f"Modified reply {'sent' if not mvp_agent.DRY_RUN else 'approved (DRY RUN)'}"}
        
        elif action.action == "reject":
            # Just remove from pending without sending
            mvp_agent.PENDING_REVIEW.pop(email_index)
            mvp_agent.save_state()
...
async def run_test_cycle():
    ...
    try:
        mvp_agent.run_cycle()
        return {"message": "Test cycle completed successfully"}
    except Exception as e:
